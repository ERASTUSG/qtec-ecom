@if (count($providers) !== 0)
    ...
    <ul class="list-inline social-login">
        @if (setting('facebook_login_enabled'))
            <li>
                <a href="{{ route('login.redirect', ['provider' => 'facebook']) }}" class="facebook" title="{{ trans('user::auth.facebook') }}">
        ...
        @if (setting('google_login_enabled'))
            <li>
                <a href="{{ route('login.redirect', ['provider' => 'google']) }}" class="google" title="{{ trans('user::auth.google') }}">
